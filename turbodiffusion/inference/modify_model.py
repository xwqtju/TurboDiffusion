    ...
    for module in model.modules():
        if type(module) is WanSelfAttention2pt1 or type(module) is WanSelfAttention2pt2:
            if attention_type == "sla":
                local_attn = SLA(head_dim=module.dim // module.num_heads, topk=sla_topk, BLKQ=128, BLKK=64)
            elif attention_type == "sagesla":
                local_attn = SageSLA(head_dim=module.dim // module.num_heads, topk=sla_topk)
            if sla_q_2to4:
