...
def replace_attention_with_sla(model: torch.nn.Module, sla_topk: float, sla_q_2to4: bool = False):
    for module in model.modules():
        if type(module) is WanSelfAttention:
            local_attn = SparseLinearAttention(head_dim=module.head_dim, topk=sla_topk)
            if sla_q_2to4:
    ...
    def build_net(self, net_dict: LazyDict, replace_sla=False):
        ...
        with misc.timer("Creating PyTorch model"):
            ...
            if replace_sla:
                ...
                replace_attention_with_sla(net, self.config.sla_topk, self.config.sla_q_2to4)
