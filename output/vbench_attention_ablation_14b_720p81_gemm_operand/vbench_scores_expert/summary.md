...
1. Plain SLA has the highest six-metric mean, 0.741830. The strongest sparse variant is SLA + K 2:4 at 0.739701, -0.002129 versus SLA and +0.005250 versus Original.
2. All six GEMM-operand sparse variants score above Original and below SLA by point estimate. Every paired-bootstrap 95% interval versus SLA crosses zero, so this eight-video, single-seed test does not establish a statistically reliable difference from SLA.
...
5. Q is feature-sparse immediately before linear `Q @ KV`; K is token-sparse immediately before `K.T @ V`; sparse-block Q/K is feature-sparse. Zeros are materialized in dense-layout tensors, so these are sparse-numerics rather than sparse-kernel speed experiments.
